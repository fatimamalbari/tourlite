...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <html lang="en" className="dark">
      <body style={theme.vars as React.CSSProperties}>
        <TourProvider enableBuilder={false}>
          <DashboardShell theme={theme} activeNav={activeNav} setActiveNav={setActiveNav}>
            {children}
          </DashboardShell>
        </TourProvider>
      </body>
...
function DashboardShell({ 
  ...
  const NAV = [
    ...
    { id: "tours", label: "Tours", icon: Route, href: "/" },
    { id: "scans", label: "Scans", icon: ScanLine, href: "/" },
    { id: "analytics", label: "Analytics", icon: BarChart3, href: "/" },
  ];

  return (
    <div className="flex h-dvh w-full overflow-hidden bg-[var(--tl-bg)] text-[var(--tl-fg)] antialiased">
      <GenerateTourModal 
        isOpen={isModalOpen} 
